...
const getAllDialogues = async (req, res) => {
   const {role}=req.params
    let dialogues = await Dialogues.find({}).lean()
    if (!dialogues?.length) {
...
const getDialogueByUserId = async (req, res) => {
    const { id } = req.params
    const dialogue = await Dialogues.find({userId:id}).lean()
...
module.exports = { getAllDialogues, createNewDialogue, upDateDialogue, getDialogueByUserId ,upDateDialogueRead}